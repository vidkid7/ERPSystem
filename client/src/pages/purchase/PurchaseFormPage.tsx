...
const PurchaseFormPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (isEdit) {
      api.get(`/purchase/purchaseinvoice/${id}`).then((res) => {
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (isEdit) {
        await api.put(`/purchase/purchaseinvoice/${id}`, { ...payload, id: Number(id) });
        message.success('Purchase invoice updated');
      } else {
        await api.post('/purchase/purchaseinvoice', payload);
