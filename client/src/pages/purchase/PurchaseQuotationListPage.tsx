...
const PurchaseQuotationListPage: React.FC = () => {
  ...
  const fetchData = async (p = page, s = search) => {
    setLoading(true);
    try {
      const res = await api.get('/purchase/quotation', { params: { search: s, page: p, pageSize: 20 } });
  ...
  return (
    <ListPage<PurchaseQuotation>
      title="Purchase Quotations" columns={columns} dataSource={data} loading={loading}
      total={total} page={page}
      onSearch={(s) => { setSearch(s); fetchData(1, s); }}
      onPageChange={(p) => { setPage(p); fetchData(p); }}
      onRefresh={() => fetchData()}
      onAdd={() => navigate('/purchase/quotation/new')}
