...
const AssetDisposalPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    form.setFieldsValue({ transactionDate: dayjs(), transactionType: 6 });
  }, []);
  ...
  const handleSubmit = async () => {
    try {
      ...
      values.transactionType = 6; // Disposal
      setLoading(true);
      await api.post('/assettransaction', values);
      message.success('Asset disposal recorded successfully');
  ...
  return (
    <FormPage title="Asset Disposal" loading={loading} onSubmit={handleSubmit} backPath="/assets/asset">
      <Form form={form} layout="vertical">
        ...
        <Form.Item name="transactionDate" label="Disposal Date" rules={[{ required: true }]}>
          <DatePicker style={{ width: '100%' }} />
        </Form.Item>
        <Form.Item name="amount" label="Disposal/Salvage Value">
          <InputNumber style={{ width: '100%' }} min={0} precision={2} />
        </Form.Item>
        <Form.Item name="documentNo" label="Document No">
          <Input />
        </Form.Item>
        <Form.Item name="remarks" label="Disposal Reason">
