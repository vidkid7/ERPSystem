...
const AssetInwardPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    form.setFieldsValue({ transactionDate: dayjs(), transactionType: 0 });
  }, []);
  ...
  const handleSubmit = async () => {
    try {
      ...
      values.transactionType = 0; // Inward
      setLoading(true);
      await api.post('/assettransaction', values);
      message.success('Asset inward recorded successfully');
  ...
  return (
    <FormPage title="Asset Inward" loading={loading} onSubmit={handleSubmit} backPath="/assets/asset">
      <Form form={form} layout="vertical">
        ...
        <Form.Item name="transactionDate" label="Inward Date" rules={[{ required: true }]}>
          <DatePicker style={{ width: '100%' }} />
        </Form.Item>
        <Form.Item name="amount" label="Cost/Value">
...
export default AssetInwardPage;
