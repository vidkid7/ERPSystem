...
const AssetTransferPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    form.setFieldsValue({ transactionDate: dayjs(), transactionType: 2 });
  }, []);
  ...
  const handleSubmit = async () => {
    try {
      ...
      values.transactionType = 2; // Transfer
      values.amount = 0;
      setLoading(true);
  ...
  return (
    <FormPage title="Asset Transfer" loading={loading} onSubmit={handleSubmit} backPath="/assets/asset">
      <Form form={form} layout="vertical">
        ...
        <Form.Item name="fromEmployeeId" label="Transfer From (Employee)" rules={[{ required: true, message: 'From employee is required' }]}>
          <Select placeholder="Select from employee" showSearch optionFilterProp="children"
            options={employees.map(e => ({ label: e.name, value: e.id }))} />
        </Form.Item>
        <Form.Item name="toEmployeeId" label="Transfer To (Employee)" rules={[{ required: true, message: 'To employee is required' }]}>
          <Select placeholder="Select to employee" showSearch optionFilterProp="children"
            options={employees.map(e => ({ label: e.name, value: e.id }))} />
        </Form.Item>
        <Form.Item name="transactionDate" label="Transfer Date" rules={[{ required: true }]}>
