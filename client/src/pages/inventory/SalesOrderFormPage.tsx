...
const SalesOrderFormPage: React.FC = () => {
  ...
  const handleSubmit = async (values: any) => {
    setLoading(true);
    try { await api.post('/inventory/sales-orders', values); navigate(-1); }
  ...
  return (
    <Card title="New Sales Order">
      <Form form={form} layout="vertical" onFinish={handleSubmit}>
        ...
        <Form.Item name="orderNo" label="Order No" rules={[{ required: true, message: 'Required' }]}>
          <Input />
        </Form.Item>
        <Form.Item name="date" label="Date" rules={[{ required: true, message: 'Required' }]}>
          <DatePicker style={{ width: '100%' }} />
        </Form.Item>
        <Form.Item name="discountPct" label="Discount %" rules={[]}>
