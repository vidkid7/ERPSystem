...
const PendingIndentFormPage: React.FC = () => {
  const [data, setData] = useState<any[]>([]);
  const [loading, setLoading] = useState(false);
  const [dates, setDates] = useState<any>(null);
  const columns = [
    { title: 'Indent No', dataIndex: 'indentNo', key: 'indentNo' },
  ...
  const handleSearch = async () => {
    ...
    try {
      const params = dates
        ...
        : {};
      const r = await api.get('/inventory/pending-indents', { params });
      setData(r.data?.Data || []);
    } catch (e) { setData([]); }
...
export default PendingIndentFormPage;
