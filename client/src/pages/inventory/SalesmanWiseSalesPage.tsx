...
const SalesmanWiseSalesPage: React.FC = () => {
  const [data, setData] = useState<any[]>([]);
  const [loading, setLoading] = useState(false);
  const [dates, setDates] = useState<any>(null);
  const columns = [
    { title: 'Salesman', dataIndex: 'salesman', key: 'salesman' },
    { title: 'Sales Amount', dataIndex: 'salesAmount', key: 'salesAmount', align: 'right' as const },
  ...
  const handleSearch = async () => {
    ...
    try {
      const params = dates
        ...
        : {};
      const r = await api.get('/inventory/salesman-wise-sales', { params });
      setData(r.data?.Data || []);
    } catch (e) { setData([]); }
...
export default SalesmanWiseSalesPage;
