...
const SalesQuotationFormPage: React.FC = () => {
  const [form] = Form.useForm();
  const [loading, setLoading] = useState(false);
  const navigate = useNavigate();
  const handleSubmit = async (values: any) => {
    setLoading(true);
    try { await api.post('/inventory/sales-quotations', values); navigate(-1); }
    catch (e) {} finally { setLoading(false); }
  };
  return (
    <Card title="New Sales Quotation">
      <Form form={form} layout="vertical" onFinish={handleSubmit}>
        ...
        <Form.Item name="validUntil" label="Valid Until" rules={[]}>
          <DatePicker style={{ width: '100%' }} />
        </Form.Item>
...
export default SalesQuotationFormPage;
