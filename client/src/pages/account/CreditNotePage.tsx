...
const CreditNotePage: React.FC = () => {
  ...
  const handleSubmit = async () => {
    try {
      ...
      await api.post('/account/credit-note', payload);
      message.success('Credit note created successfully');
      navigate('/account/vouchers');
    } catch (err: any) {
      if (err.errorFields) return;
      message.error(err.response?.data?.responseMSG || 'Failed to save credit note');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <FormPage title="Credit Note" loading={loading} onSubmit={handleSubmit} backPath="/account/vouchers">
