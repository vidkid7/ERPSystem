...
const JournalVoucherListPage: React.FC = () => {
  ...
  const handleSearch = async () => {
    setLoading(true);
    try { const res = await api.get('/account/journal-voucher-list'); setData(res.data?.Data || []); }
    catch { setData([]); } finally { setLoading(false); }
  };
  return (
    <Card title="Journal Voucher List" extra={<Space><RangePicker /><Button type="primary" onClick={handleSearch}>Search</Button></Space>}>
