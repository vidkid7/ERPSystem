...
const DayBookPage: React.FC = () => {
  ...
  const handleSearch = async () => {
    setLoading(true);
    try { const res = await api.get('/account/day-book'); setData(res.data?.Data || []); }
    catch { setData([]); } finally { setLoading(false); }
  };
  return (
    <Card title="Day Book" extra={<Space><RangePicker /><Button type="primary" onClick={handleSearch}>Search</Button></Space>}>
