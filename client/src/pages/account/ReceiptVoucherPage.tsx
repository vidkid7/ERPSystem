...
const ReceiptVoucherPage: React.FC = () => {
  ...
  const handleSubmit = async () => {
    try {
      ...
      await api.post('/account/voucher/receipt', payload);
      message.success('Receipt voucher created successfully');
      navigate('/account/vouchers');
    } catch (err: any) {
      if (err.errorFields) return;
      message.error(err.response?.data?.responseMSG || 'Failed to save receipt voucher');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <FormPage title="Receipt Voucher" loading={loading} onSubmit={handleSubmit} backPath="/account/vouchers">
      <Form form={form} layout="vertical" initialValues={{ paymentMode: 'Cash' }}>
        <Form.Item name="receivedFrom" label="Received From" rules={[{ required: true, message: 'Please select party' }]}>
