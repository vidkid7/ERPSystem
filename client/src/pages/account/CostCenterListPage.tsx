...
const CostCenterListPage: React.FC = () => {
  ...
  const fetchData = async (p = page, s = search) => {
    setLoading(true);
    try {
      const res = await api.get('/api/costcenter', { params: { search: s, page: p, pageSize: 20 } });
  ...
  return (
    <ListPage<CostCenter>
      title="Cost Centers" columns={columns} dataSource={data} loading={loading}
