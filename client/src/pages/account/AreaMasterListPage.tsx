...
const AreaMasterListPage: React.FC = () => {
  const [data, setData] = useState<AreaMaster[]>([]);
  ...
  const fetchData = async (p = page, s = search) => {
    setLoading(true);
    try {
      const res = await api.get('/api/areamaster', { params: { search: s, page: p, pageSize: 20 } });
  ...
  return (
    <ListPage<AreaMaster>
      title="Area Masters" columns={columns} dataSource={data} loading={loading}
