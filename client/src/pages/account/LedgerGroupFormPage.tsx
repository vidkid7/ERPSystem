...
const LedgerGroupFormPage: React.FC = () => {
  const [form] = Form.useForm();
  const [loading, setLoading] = useState(false);
  const handleSubmit = async (values: any) => {
    setLoading(true);
    try { await api.post('/account/ledger-group', values); message.success('Saved successfully'); form.resetFields(); }
    catch { message.error('Save failed'); } finally { setLoading(false); }
  };
  return (
    <Card title="Ledger Group Form">
      <Form form={form} layout="vertical" onFinish={handleSubmit}>
        <Row gutter={16}>
          ...
          <Form.Item name="parentGroup" label="Parent Group"><Select placeholder="Select Parent Group" /></Form.Item>
...
export default LedgerGroupFormPage;
