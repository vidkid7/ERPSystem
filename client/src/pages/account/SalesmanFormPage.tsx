...
const SalesmanFormPage: React.FC = () => {
  const [form] = Form.useForm();
  const [loading, setLoading] = useState(false);
  const handleSubmit = async (values: any) => {
    setLoading(true);
    try { await api.post('/account/salesman', values); message.success('Saved successfully'); form.resetFields(); }
    catch { message.error('Save failed'); } finally { setLoading(false); }
  };
  return (
    <Card title="Salesman Form">
      <Form form={form} layout="vertical" onFinish={handleSubmit}>
        <Row gutter={16}>
          ...
          <Form.Item name="phone" label="Phone"><Input placeholder="Enter Phone" /></Form.Item>
          <Form.Item name="email" label="Email"><Input placeholder="Enter Email" /></Form.Item>
          <Form.Item name="territory" label="Territory"><Select placeholder="Select Territory" /></Form.Item>
        </Row>
