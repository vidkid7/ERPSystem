...
interface FinalReportsRow {
  id: number;
  description: string;
  amount: number;
}

const FinalReportsPage: React.FC = () => {
  const [loading, setLoading] = useState(false);
  const [data, setData] = useState<FinalReportsRow[]>([]);

  const columns: ColumnsType<FinalReportsRow> = [
