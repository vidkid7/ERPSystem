...
const SpecimenListPage: React.FC = () => {
  ...
  const fetchData = async (p = page, s = search) => {
    setLoading(true);
    try {
      const res = await api.get('/specimen', { params: { search: s, page: p, pageSize: 20 } });
  ...
  return (
    <ListPage<Specimen>
      title="Specimens" columns={columns} dataSource={data} loading={loading}
