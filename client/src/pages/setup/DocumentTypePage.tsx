...
const DocumentTypePage: React.FC = () => {
  ...
  const fetchData = async (p = page, s = search) => {
    setLoading(true);
    try {
      const res = await api.get('/api/documenttype', { params: { search: s, page: p, pageSize: 20 } });
  ...
  return (
    <ListPage<DocumentType>
      title="Document Types" columns={columns} dataSource={data} loading={loading}
